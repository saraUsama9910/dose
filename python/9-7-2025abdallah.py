...
def process_dicom_files(files):
    ...
    for key in ct_slices_by_case:
        ...
        total_ctdi_vals.extend(ctdi_vals)  # نجمع كل CTDI لكل الصور
        ...
        if len(z_positions) >= 2:
            ...
            scan_length = 400  # fallback

        total_scan_length += scan_length  # نجمع طول المسح لكل حالة

    # نحسب المتوسط العام لكل الصور
    if total_ctdi_vals:
        avg_ctdi = sum(total_ctdi_vals) / len(total_ctdi_vals)
    else:
        avg_ctdi = 0

    dlp = avg_ctdi * total_scan_length

    # نأخذ وصف دراسة أول حالة (أو يمكن تعديل ذلك)
    study_desc = temp_cases[next(iter(ct_slices_by_case))]["Dataset"].get("StudyDescription", "").lower()
    k = 0.015
    ...
    # يمكن تخزين النتائج الكلية في متغير جديد أو طباعتها
    overall_dose_report = {
        "CTDIvol": round(avg_ctdi, 5),
        "TotalScanLength": round(total_scan_length, 2),
        "DLP": round(dlp, 5),
        "kFactor": k,
        "mSv": round(dose, 5)
    }

    print("Total CT Dose Report:", overall_dose_report)
