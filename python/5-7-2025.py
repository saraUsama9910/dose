...
def process_dicom_files(files):
    ...
    for path in files:
        try:
            ...
            if modality == "CT":
                if "CT_DATA" not in temp_cases[key]:
                    temp_cases[key]["CT_DATA"] = {
                        "ctdi_list": [],
                        "thickness_list": [],
                        "num_slices": 0
                    }
                ...
                if isinstance(ctdi, (float, int)):
                    temp_cases[key]["CT_DATA"]["ctdi_list"].append(float(ctdi))
                if isinstance(thickness, (float, int)):
                    temp_cases[key]["CT_DATA"]["thickness_list"].append(float(thickness))
                    temp_cases[key]["CT_DATA"]["num_slices"] += 1
                ...
                if dap:
                    ...
                    temp_cases[key]["DLP"] = case_dap_totals[key]
    ...
    # ==== CT Dose Calculation Enhanced ====
    for key, case in temp_cases.items():
        if case["Modality"] == "CT":
            ds = case["Dataset"]

            # محاولة قراءة DLP من التاج مباشرة
            try:
                dlp_tag = ds.get((0x0018, 0x9343), None)
                dlp = float(dlp_tag) if dlp_tag else None
            except:
                dlp = None

            if not dlp and "CT_DATA" in case:
                ctdis = case["CT_DATA"]["ctdi_list"]
                thicknesses = case["CT_DATA"]["thickness_list"]
                num_slices = case["CT_DATA"]["num_slices"]

                if ctdis and thicknesses and num_slices > 0:
                    avg_ctdi = sum(ctdis) / len(ctdis)
                    avg_thickness = sum(thicknesses) / len(thicknesses)
                    scan_length = avg_thickness * num_slices
                    dlp = avg_ctdi * scan_length
                    case["CTDIvol"] = avg_ctdi
                else:
                    dlp = 0
                    case["CTDIvol"] = 0
            else:
                case["CTDIvol"] = float(ds.get("CTDIvol", 0))

            # معامل التحويل k حسب نوع الفحص
            k = 0.015
            ...
            if "head" in study_desc:
                ...
                k = 0.014
            elif "abdomen" in study_desc and "pelvis" in study_desc:
                k = 0.015
            elif "abdomen" in study_desc:
                k = 0.017
            elif "pelvis" in study_desc:
    ...
    # الجرعة التراكمية والسنوية لكل مريض
    patient_records = {}
...
def display_text_data():
    ...
    for patient_id in patient_records:
        ...
        for current_date, _ in records:
            ...
            total_dose_year = sum(dose for date, dose in records if start_date <= date <= current_date)
    ...
    if sort_option == "Date":
        ...
        col_indices = {
            "select": 0, "date": 1, "name": 2, "patientid": 3,
            "modality": 4, "dose": 5, "accumulated": 6, "per_year": 7
        }
    else:
        ...
        col_indices = {
            "select": 0, "name": 1, "date": 2, "patientid": 3,
            "modality": 4, "dose": 5, "accumulated": 6, "per_year": 7
        }
...
ctk.CTkButton(root, text="📁 DICOM Folder", command=read_dicom_folder,
              width=140, height=40, fg_color=BUTTON_COLOR, hover_color=BUTTON_HOVER,
              corner_radius=10, font=("Arial", 16, "bold")).place(relx=0.01, rely=0.28)
